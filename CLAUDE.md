...
**CDN offload** — for external callers, `getBlob` returns a 307 redirect to a cacheable, content-addressed blob origin instead of streaming. When `CDN_DOMAIN` is set it redirects to `https://{CDN_DOMAIN}/{name}/blobs/{digest}` (a separate CDN host expected to proxy its root to the registry's `/_cdn/` path; the cross-origin hop also drops the client's `Authorization` header). When `CDN_DOMAIN` is empty it redirects to the same-host relative path `/_cdn/{name}/blobs/{digest}`, so a CDN sitting directly in front of the registry (e.g. Google Cloud CDN on `registry.deploys.app`, caching only `/_cdn/*`) serves the blob. Either way the `/_cdn/` handler serves blobs unauthenticated — blobs are content-addressed by digest, so this never sits in front of the authenticated `/v2/` path. Internal callers (private/loopback/link-local `X-Real-Ip`) bypass the redirect and stream directly — this keeps in-cluster pulls off the CDN path.
